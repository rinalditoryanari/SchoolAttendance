...
<div class="pd-20 card-box mb-30">
    <div class="clearfix">
        <div class="pull-left">
            <h4 class="text-blue h4">Form Asdos</h4>
        </div>
        <div class="pull-right">
        </div>
    </div>
    <form method="POST" action="{{route('admin.asdos.add')}}" enctype="multipart/form-data">
        @csrf
        <div class="row">
            <div class="col-md-6 col-sm-12">
                ...
                <div class="form-group @error('firstName') has-danger @enderror">
                    <label for="firstName" class="form-control-label">Nama Depan Asdos</label>
                ...
                <div class="form-group @error('lastName') has-danger @enderror">
                    <label for="lastName" class="form-control-label">Nama Belakang Asdos</label>
                ...
                <div class="form-group @error('status') has-danger @enderror">
                    ...
                    <select name="status" id="status" class="selectpicker form-control" data-style="btn-outline-primary">
                        ...
                        <option {{ old('status') == 'mahasiswa' ? 'selected' : '' }} value="mahasiswa">Mahasiswa</option>
                        <option {{ old('status') == 'non mahasiswa' ? 'selected' : '' }} value="non mahasiswa">Non Mahasiswa</option>
                    </select>
                    @error('status')
                    <div class="form-control-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group @error('dosen_id') has-danger @enderror">
                    ...
                    <select name="dosen_id" id="dosen_id" class="selectpicker form-control" data-style="btn-outline-primary">
                        ...
                        <option {{ old('dosen_id') ==  $dosen->id  ? 'selected' : '' }} value="{{ $dosen->id }}">{{ $dosen ->firstName }} {{ $dosen ->lastName }}</option>
                        @endforeach
                    </select>
                    @error('dosen_id')
                    <div class="form-control-feedback">{{ $message }}</div>
