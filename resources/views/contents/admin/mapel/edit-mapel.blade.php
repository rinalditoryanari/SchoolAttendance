...
<div class="pd-20 card-box mb-30">
    ...
    <form method="POST" action="{{ route('admin.mapel.edit', ['mapel' => $mapel->id]) }}" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="row">
            <div class="col-md-6 col-sm-12">
                <div class="form-group @error('kelas_id') has-danger @enderror">
                    ...
                    <select required name="kelas_id" id="kelas_id" class="selectpicker form-control" data-style="btn-outline-primary">
                        <option selected disabled hidden>Pilih Mapel Kelas...</option>
                        @foreach ($kelas as $kelas)
                        @if (old('kelas_id', $mapel->kelas_id) == $kelas->id)
                ...
                <div class="form-group @error('dosen_id') has-danger @enderror">
                    ...
                    <select required name="dosen_id" id="dosen_id" class="selectpicker form-control" data-style="btn-outline-primary">
                        ...
                        @if (old('dosen_id', $mapel->dosen_id) == $dosen->id)
                ...
                <div class="form-group @error('kode') has-danger @enderror">
                    ...
                    <input value="{{ old('kode', $mapel->kode) }}" name="kode" id="kode" type="text" class="form-control @error('kode') form-control-danger @enderror" autofocus required>
                    @error('kode')
                    <div class="form-control-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group @error('nama') has-danger @enderror">
                    ...
                    <input value="{{ old('nama', $mapel->nama) }}" name="nama" id="nama" type="text" class="form-control @error('nama') form-control-danger @enderror" required>
                    @error('nama')
                    <div class="form-control-feedback">{{ $message }}</div>
                    @enderror
                </div>
